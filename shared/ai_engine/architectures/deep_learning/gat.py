"""Candidat GAT, entraîné avec une attention de graphe Keras native (`GATLayer`).
...
`DatasetArtifact`), le choix Keras natif plutôt que `torch_geometric`, et la limitation
d'attention dense O(n²) (acceptable pour des graphes de taille modérée uniquement).
...
+ scheduler, epochs) — autorité unique partagée avec `GNNModel`/`GraphSAGEModel`.
"""
...
from shared.ai_engine.architectures.deep_learning.graph_layers import GATLayer
...
class GATModel(UntrainedModel):
    candidate_id = "gat"
    n_trials = 12

    def train(self, dataset: DatasetArtifact) -> Any:
        frame = load_dataframe(dataset)
        return search_and_train_graph_model(GATLayer, frame, n_trials=self.n_trials)
