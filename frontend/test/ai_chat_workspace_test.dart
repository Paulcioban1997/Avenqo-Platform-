...
void main() {
  testWidgets('renders the empty business chat state', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(home: AssistantPage(api: chatApi(client))),
    );
  ...
  testWidgets('creates a conversation and renders Central AI response', (
    tester,
  ) async {
    final client = MockClient((request) async {
      ...
      if (request.url.path.endsWith('/conversations')) {
        return http.Response(
          '{"id":"chat-1","title":"Sales","created_at":"2026-08-18T12:00:00Z","updated_at":"2026-08-18T12:00:00Z"}',
          201,
        );
      }
  ...
  testWidgets('renders structured Coming Soon response', (tester) async {
    final client = MockClient((request) async {
      ...
      if (request.url.path.endsWith('/conversations')) {
        return http.Response(
          '{"id":"chat-2","title":"CRM","created_at":"2026-08-18T12:00:00Z","updated_at":"2026-08-18T12:00:00Z"}',
          201,
        );
      }
      return http.Response(
  ...
  testWidgets('offers credit management when AI credits are exhausted', (
    tester,
  ) async {
    final client = MockClient((request) async {
      if (request.method == 'GET') return http.Response('[]', 200);
      if (request.url.path.endsWith('/conversations')) {
        return http.Response(
          '{"id":"chat-3","title":"Sales","created_at":"2026-08-18T12:00:00Z","updated_at":"2026-08-18T12:00:00Z"}',
          201,
        );
      }
      return http.Response(
    ...
    expect(
      find.text('Your AI credits are exhausted. Add credits to continue.'),
      findsOneWidget,
    );
