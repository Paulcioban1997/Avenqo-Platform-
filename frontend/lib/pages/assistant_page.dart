...
class _AssistantPageState extends State<AssistantPage> {
  ...
  Future<void> _send([String? suggestedText]) async {
    ...
    try {
      ...
      _stream = _chat.streamMessage(conversation.id, content).listen(
        _handleStreamEvent,
        onError: _completeStreamWithError,
  ...
  void _completeStreamWithError(Object error) {
    if (!mounted || _messages.isEmpty) return;
    // Le backend renvoie un message déjà sûr à afficher (voir
    // ConversationNotFoundError/AIServiceUnavailableError/AIQuotaExceededError) ;
...
class ChatComposer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(color: colors.surface, padding: const EdgeInsets.fromLTRB(20, 12, 20, 20), child: SafeArea(top: false, child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
      ...
      generating ? Tooltip(message: 'Stop generating', child: IconButton.filledTonal(onPressed: onStop, icon: const Icon(Icons.stop))) : Tooltip(message: 'Send message', child: IconButton.filled(onPressed: () => onSend(null), style: IconButton.styleFrom(backgroundColor: _Brand.blue), icon: const Icon(Icons.arrow_upward))),
    ])));
  }
}