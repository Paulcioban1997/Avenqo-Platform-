  ...
  GoRouter _createRouter() => GoRouter(
    ...
    routes: [
      ...
      ShellRoute(builder: (context, state, child) => AppShell(auth: widget.auth, currentPath: state.uri.path, child: child), routes: [
        ...
        ShellRoute(builder: (context, state, child) => RetailAgentShell(currentPath: state.uri.path, child: child), routes: [for (final destination in retailAgentDestinations) GoRoute(path: destination.path, builder: (context, state) => _retailPage(destination.path, widget.auth, productId: state.uri.queryParameters['product_id']))]),
...
Widget _retailPage(String path, AuthController auth, {String? productId}) { final companyId = auth.company?['id']; return switch (path) { '/retail' => DashboardPage(auth: auth), '/retail/sales' => SalesPage(key: ValueKey('sales-$companyId'), api: auth.api), '/retail/customers' => CustomersPage(key: ValueKey('customers-$companyId'), api: auth.api), '/retail/products' => ProductsPage(key: ValueKey('products-$companyId-$productId'), api: auth.api, initialProductId: productId), '/retail/recommendations' => RecommendationsPage(key: ValueKey('recommendations-$companyId'), api: auth.api), _ => DashboardPage(auth: auth), }; }
